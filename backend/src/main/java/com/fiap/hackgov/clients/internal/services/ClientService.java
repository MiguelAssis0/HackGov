...
public class ClientService {
    ...
    public Response update(UUID id, SaveRequest request, Employee employee) {
        ...
        validateCpf(cpf);
        String lookup = SensitiveStringConverter.lookup("client-cpf", cpf);
        repository.findByCityHall_IdAndCpfLookup(cityId(current), lookup).filter(existing -> !existing.getId().equals(id)).ifPresent(existing -> {
            throw new ResourceAlreadyExistsException("Ja existe cliente com este CPF na prefeitura");
        });
        apply(client, request, cpf, lookup);
        return toResponse(repository.save(client), current);
    ...
    private void apply(Client client, SaveRequest request, String cpf, String lookup) {
        ...
        client.setNickname(clean(request.nickname()));
        client.setCpf(cpf);
        client.setCpfLookup(lookup);
        client.setPhone(clean(request.phone()));
        client.setSecondaryContact(clean(request.secondaryContact()));
        client.setAddress(clean(request.address()));
        client.setStateRegistration(clean(request.stateRegistration()));
    ...
    private ServiceResponse serviceResponse(ClientServiceRecord record, boolean sensitive) {
        return new ServiceResponse(record.getId(), record.getArea(), record.getDescription(), sensitive ? record.getObservation() : maskGeneric(record.getObservation()), record.getServiceDate(), record.getCreatedBy() == null ? null : record.getCreatedBy().getFullName(), record.getCreatedAt());
    }

    private Client findScoped(UUID id, Employee employee) {
        return repository.findByIdAndCityHall_Id(id, cityId(employee)).orElseThrow(() -> new ResourceNotFoundException("Cliente nao encontrado"));
    }
    ...
    private void validateCpf(String cpf) {
        ...
        for (int digit = 9; digit < 11; digit++) {
            int sum = 0;
            for (int index = 0; index < digit; index++) sum += (cpf.charAt(index) - '0') * (digit + 1 - index);
            int check = (sum * 10) % 11;
            if (check == 10) check = 0;
            if (check != cpf.charAt(digit) - '0') throw new BusinessException("CPF invalido");
        }
    }

    private String formatCpf(String cpf) {
        return cpf == null ? "" : cpf.replaceFirst("(\\d{3})(\\d{3})(\\d{3})(\\d{2})", "$1.$2.$3-$4");
    }

    private String maskCpf(String cpf) {
        return cpf == null || cpf.length() != 11 ? "***" : String.format("***.%s.%s-**", cpf.substring(3, 6), cpf.substring(6, 9));
    }
    ...
    private String maskAddress(String value) {
        return value == null || value.isBlank() ? "" : "Endereco protegido";
    }
