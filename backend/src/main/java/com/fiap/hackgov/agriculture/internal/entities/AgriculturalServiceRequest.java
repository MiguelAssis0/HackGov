...
@NoArgsConstructor
@Entity
@Table(name = "agricultural_service_requests", uniqueConstraints = @UniqueConstraint(name = "agri_protocol_city_uk", columnNames = {"city_hall_id", "protocol"}))
public class AgriculturalServiceRequest {
    ...
    @ManyToOne(optional = false, fetch = FetchType.LAZY)
    private AgriculturalServiceType serviceType;
    ...
    @Column(precision = 12, scale = 2)
    private BigDecimal funderAmount;
    private boolean donation;
    @Convert(converter = SensitiveStringConverter.class)
    @Column(length = 1000)
    private String donationOrigin = "";
    @Lob
    ...
    public LocalDate expirationDate() {
        return paymentDate == null ? null : paymentDate.plusMonths(6);
    }

    public boolean shouldExpire(LocalDate today) {
        return expirationDate() != null && !expirationDate().isAfter(today) && status != Status.CANCELLED && status != Status.EXPIRED;
    }
}
