...
function Kpi({ icon, label, value }) { return <article className="gestao-kpi-card"><i className={`bi ${icon}`}></i><span>{label}</span><strong>{typeof value === "number" ? number(value) : value}</strong></article>; }
...
function PredictiveAnalysis({ prediction, loading, error }) {
  if (loading) return <section className="gestao-panel gestao-predictive-empty"><i className="bi bi-stars"></i><strong>Gerando análise preditiva...</strong><span>A IA está analisando apenas os indicadores deste período.</span></section>;
  if (error) return <section className="gestao-panel gestao-filter-alert" role="alert"><i className="bi bi-exclamation-triangle"></i> {error}</section>;
  if (!prediction) return <section className="gestao-panel gestao-predictive-empty"><i className="bi bi-stars"></i><strong>Selecione a aba para gerar a análise</strong><span>A análise usa os mesmos filtros dos indicadores.</span></section>;

  const forecast = prediction.forecast;
  return <>
    <section className="gestao-panel gestao-predictive-hero"><div className="gestao-section-heading"><div><p className="eyebrow dark mb-0">IA aplicada aos dados do sistema</p><h2>Leitura do próximo período</h2></div><span className={`gestao-ai-badge ${prediction.aiGenerated ? "is-live" : ""}`}><i className="bi bi-stars"></i> {prediction.aiGenerated ? "Recomendação gerada pela IA" : "Recomendação de contingência"}</span></div><p className="gestao-predictive-intro">A projeção é calculada sobre tarefas concluídas, pontos, série temporal e setores visíveis neste filtro. Ela é uma indicação operacional, não uma garantia.</p><div className="gestao-predictive-kpis"><Kpi icon="bi-graph-up-arrow" label="Tarefas projetadas" value={forecast.projectedTasks} /><Kpi icon="bi-gem" label="Pontos projetados" value={forecast.projectedPoints} /><Kpi icon="bi-speedometer2" label="Tendência" value={`${number(forecast.trendPercent)}%`} /><Kpi icon="bi-shield-check" label="Confiança" value={forecast.confidence} /></div></section>
    <div className="gestao-chart-grid"><section className="gestao-panel" aria-labelledby="previsao-grafico-title"><div className="gestao-section-heading"><div><p className="eyebrow dark mb-0">Projeção</p><h2 id="previsao-grafico-title">Próximos períodos</h2></div><span className={`gestao-trend gestao-trend-${forecast.trend}`}>{forecast.trend}</span></div><LineChart values={forecast.nextPeriods} label="Previsão de tarefas dos próximos períodos" /></section><section className="gestao-panel" aria-labelledby="sinais-title"><div className="gestao-section-heading"><div><p className="eyebrow dark mb-0">Alertas e oportunidades</p><h2 id="sinais-title">Sinais encontrados</h2></div></div><div className="gestao-signal-list">{prediction.signals.map((signal) => <article className={`gestao-signal is-${signal.severity}`} key={`${signal.severity}-${signal.title}`}><i className={`bi ${signal.severity === "warning" ? "bi-exclamation-triangle" : signal.severity === "positive" ? "bi-check-circle" : "bi-info-circle"}`}></i><div><strong>{signal.title}</strong><span>{signal.detail}</span></div></article>)}</div></section></div>
    <section className="gestao-panel gestao-recommendations" aria-labelledby="recommendations-title"><div className="gestao-section-heading"><div><p className="eyebrow dark mb-0">Plano sugerido</p><h2 id="recommendations-title">Recomendações da IA</h2></div><span className="gestao-ai-scope">Somente dados agregados do relatório</span></div><div className="gestao-recommendations-text">{prediction.recommendations}</div><small className="gestao-chart-note">A IA não recebeu nomes, IDs ou dados fora dos indicadores desta prefeitura e deste período.</small></section>
  </>;
}
...
export default function ManagementPage() {
  ...
  const [activeTab, setActiveTab] = useState(() => new URLSearchParams(window.location.search).get("aba") === "preditiva" ? "predictive" : "overview");
  ...
  useEffect(() => {
    if (activeTab !== "predictive") return undefined;
    let mounted = true;
    setPredictionLoading(true);
    setPredictionError("");
    api.getManagementPrediction(query).then((response) => {
      if (mounted) setPrediction(response);
    }).catch((error) => {
      if (mounted) setPredictionError(error.message);
    }).finally(() => {
  ...
  // ponytail: filtro reativo — período/setor aplicam sozinhos, sem botão
  useEffect(() => {
    if (form.period === "personalizado" && (!form.start || !form.end)) return;
    setQuery({ period: form.period, sectorId: form.sectorId, start: form.start, end: form.end });
  }, [form.period, form.sectorId, form.start, form.end]);
  ...
  return <DashboardLayout styles={["/css/gestao.css"]}><main className="dashboard"><div className="container-fluid gestao-page">
    ...
    </> : <PredictiveAnalysis prediction={prediction} loading={predictionLoading} error={predictionError} />}
