  ...
  return (
    <DashboardLayout styles={["/css/tarefas.css"]}>
      <div className="container">
        ...
        <div className="task-filters" style={{display:"grid", gridTemplateColumns:"1.2fr 0.9fr 1.1fr auto", gap:".75rem", alignItems:"end"}}>
          ...
          <div className="task-filter-actions" style={{display:"flex", gap:".5rem"}}><input className="field-input" placeholder="Buscar..." value={query} onChange={e=> setQuery(e.target.value)} style={{minWidth:160}}/><button className="btn btn-outline-secondary" onClick={()=> {setFiltros({responsavel:"", prioridade:"", origem:""}); setQuery("");}}>Limpar</button></div>
        </div>
                ...
                {col.tarefas.map(tarefa=>{
                  ...
                  return (
                    <article key={tarefa.id} className="task-card">
                      ...
                      <h4><button className="task-title-link" style={{background:"none",border:0,padding:0, font:"inherit", color:"inherit", cursor:"pointer"}} onClick={()=> setDetailTask(tarefa)}>{tarefa.title}</button></h4>
                      ...
                      <div className="task-card-actions">
                        ...
                        <label className="task-move-form" style={{flex:"1 1 145px"}}><select value={tarefa.status} onChange={e=> moverStatus(tarefa, e.target.value)} aria-label="Mover tarefa"><option value="TODO">A fazer</option><option value="IN_PROGRESS">Em andamento</option><option value="IN_REVIEW">Em revisão</option><option value="COMPLETED">Concluída</option></select></label>
                      </div>
                    </article>
                  );
                })}
        ...
        <section className="task-requests" style={{display:"grid", gridTemplateColumns:"repeat(2, minmax(0,1fr))", gap:"1rem", marginTop:"1rem"}}>
