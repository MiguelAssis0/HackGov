...
export default function TasksPage(){
  ...
  // ponytail: origem real = setor do funcionário logado; backend usa ela e rejeita destino igual
  const norm=(v)=> String(v||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().trim();
  const mySectorId=useMemo(()=>{
    const name=currentEmployee?.sectorName || currentEmployee?.sector?.name || (typeof currentEmployee?.sector==="string"? currentEmployee.sector : "") || user?.setor || user?.sector || "";
    if(!name || !sectors.length) return "";
    return sectors.find(s=> norm(sectorName(s))===norm(name))?.id || "";
  },[currentEmployee, sectors]);
  useEffect(()=>{ if(!isAdmin && mySectorId && setorAtivo!==mySectorId) setSetorAtivo(mySectorId); },[isAdmin, mySectorId]);
  // destino exclui só a origem real (setor do logado, igual ao backend); desconhecida => lista todos
  const originSectorId=mySectorId || "";
